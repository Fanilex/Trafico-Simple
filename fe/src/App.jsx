...
export default function Home() {
   ...
   const avgSpeeds = useRef([]);  // Store average speeds over time
   ...
   const handleStart = () => {
       ...
       avgSpeeds.current = [];  // Clear the array at the start
       running.current = setInterval(() => {
           fetch(`http://localhost:8000${location}?speed=${simSpeed}`) // Send speed with each request
           .then((res) => {
               if (!res.ok) {
           ...
           .then((data) => {
               ...
               // Accumulate the average velocity over time
               avgSpeeds.current.push(data['avg_velocity']);
           })
           .catch((err) => {
               console.error('Error during simulation:', err.message);
               handleStop();
           });
       }, 1000 / simSpeed);  // Adjust the interval based on the speed
   };


   // Stop function
   const handleStop = () => {
       clearInterval(running.current); // Stop the simulation


       // Check if any data was accumulated
       if (avgSpeeds.current.length === 0) {
           console.error("No velocity data accumulated.");
           return;
       }
       ...
       Plotly.newPlot('mydiv', [
           {
               y: avgSpeeds.current,  // Use the accumulated average velocities
               mode: 'lines',
               line: { color: '#80CAF6' },
           },
       ], {
           title: 'Average Velocity of Cars',
           xaxis: { title: 'Time (s)' },
           yaxis: { title: 'Average Velocity' },
       });
   };
